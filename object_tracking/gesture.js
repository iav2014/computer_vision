 ...
 * this example code using opencv to count fingers in hand
 * using webcam device
 * taken from justadudewhohacks fixed draw functions in examples + fixed draw functions optional params
 * modified by (c) Nacho Ariza Nov 2019
 * MIT License
 */
...
while (!done) {
	...
	if (frame.empty) {
		...
		detect(frame.resize(340, 500), (handMask, sideBySide) => {
			if (handMask !== null) cv.imshow('handMask', handMask);
			if (sideBySide !== null) cv.imshow('result', sideBySide);
		});
		cv.waitKey(delay);
	}
}
